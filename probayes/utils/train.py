...
from probayes.utils.misc import BlackHole
...
def log_losses(loss, loss_dict, scalar_dict, it, tag, logger=BlackHole(), writer=BlackHole(),debug=False):
    import torch
    ...
    if not debug:
        wandb.log({f'train/loss': loss}, step=it)
        for k,v in loss_dict.items():
