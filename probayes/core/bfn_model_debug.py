...
from probayes.modules.net.ga_bfn_stable import GAEncoder_BFN
import gc
...
class BFNModel_debug(nn.Module):
    def __init__(self,cfg):
        ...
        self.sample_seq = self._interpolant_cfg.sample_sequence
    ...
    def forward(self, batch):
        ...
        pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob = self.ga_encoder(
            t, rotmats_t, mu_trans_t, m_angles_t, theta_seqs_t, node_embed, edge_embed, gen_mask, res_mask, log_acc_t)
    ...
    def sample(self, batch, num_steps = 100, sample_bb=True, sample_ang=True, sample_seq=True):
        ...
        for idx in torch.range(1, num_steps+1):
            ...
            # rots
            pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob = self.ga_encoder(
                t, rotmats_t, mu_trans_t, m_angles_t, 
                theta_seqs_t, node_embed, edge_embed, 
                batch['generate_mask'].long(), batch['res_mask'].long(),log_c_t)
            pred_rotmats_1 = self.recover_gt(gen_mask, pred_rotmats_1, rotmats_1)
