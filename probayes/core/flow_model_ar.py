...
from probayes.modules.net.ga_ar import GAEncoder
...
from probayes.modules.protein.constants import AA, BBHeavyAtom, max_num_heavyatoms, aaidx2rgidx_mat, sc_rg_vocab
...
class FlowModel_AR(nn.Module):
    def __init__(self,cfg):
        ...
        self.sc_rg_vocab = sc_rg_vocab
        self.sc_rg_max_classes = max([len(v) for v in sc_rg_vocab.values()])
    ...
    def seq_to_simplex_rg(self, rg_gen, ar_time):
        K = self.Ks[ar_time+1]
        return clampped_one_hot(rg_gen, K).float() * self.k * 2 - self.k # (B,L,K)    
    
    def calc_angle_loss(self, angles_t, angles, pred_angle, norm_scale, angle_mask_loss):
        angle_mask_loss = torch.cat([angle_mask_loss,angle_mask_loss],dim=-1)
        gt_angle_vf = torus.tor_logmap(angles_t, angles)
        gt_angle_vf_vec = torch.cat([torch.sin(gt_angle_vf),torch.cos(gt_angle_vf)],dim=-1)
        pred_angle_vf = torus.tor_logmap(angles_t, pred_angle)
        ...
        angles_1_vec = torch.cat([torch.sin(angles),torch.cos(angles)],dim=-1)
        pred_angles_1_vec = torch.cat([torch.sin(pred_angle),torch.cos(pred_angle)],dim=-1)
        ...
        return angle_loss + torsion_loss
    ...
    def forward(self, batch):
        ...
        bb_angles = angles_1[:,:,0] # (B,L)
        sc_angles = angles_1[:,:,1:] # (B,L,4)
        
        rg_1 = torch.tensor(aaidx2rgidx_mat,device=seqs_1.device)[seqs_1] # (B,L,4)
        
        # ar_time = torch.randint(0, self.n_ar_steps, size=(num_batch,), device=DEVICE)
        ar_time = randint(0,self.n_ar_steps-1) # a scaler
        
        # casual mask
        ar_mask = torch.arange(self.n_ar_steps,device=DEVICE)[None,:] < ar_time
        ar_mask = ar_mask.unsqueeze(-2)

        # the first dimension of angles is not auto-regressively generated
        rg_context = rg_1 * ar_mask # (B,L,4)
        rg_gen = rg_1[:,:, ar_time].squeeze()
        sc_angles_gen = sc_angles[:,:, ar_time] # (B,L)
        sc_angles_context = sc_angles * ar_mask # (B,L,4)
        
        # prepare for denoise
        # trans_1_c,_ = self.zero_center_part(trans_1,gen_mask,res_mask)
        trans_1_c = trans_1 # already centered when constructing dataset
        
        rg_1_simplex = self.seq_to_simplex_rg(rg_gen, ar_time)
        ...
        with torch.no_grad():
            t = torch.randint(0, self.n_steps,size=(num_batch,1), 
                            device=DEVICE)/self.n_steps 
            # t_index = t * self.n_steps + 1
            if self.sample_structure:
                # corrupt trans
                trans_0 = torch.randn((num_batch,num_res,3), 
                                      device=DEVICE) * self._interpolant_cfg.trans.sigma # scale with sigma?
                ...
                # corrup angles
                bb_angles_0 = torus.tor_random_uniform(bb_angles.shape, device=DEVICE, dtype=bb_angles.dtype) # (B,L)
                bb_angles_t = torus.tor_geodesic_t(t, bb_angles, bb_angles_0)
                bb_angles_t = torch.where(batch['generate_mask'],bb_angles_t,bb_angles)
                sc_angles_gen_0 = torus.tor_random_uniform(sc_angles_gen.shape, device=DEVICE, dtype=sc_angles_gen.dtype) # (B,L)
                sc_angles_gen_t = torus.tor_geodesic_t(t, sc_angles_gen, sc_angles_gen_0)
                sc_angles_gen_t = torch.where(batch['generate_mask'],sc_angles_gen_t,sc_angles_gen)
            else:
            ...
            if self.sample_sequence:
                # corrupt seqs
                rg_0_simplex = self.k * torch.randn_like(rg_1_simplex) # (B,L)
                rg_t_simplex = ((1 - t[..., None]) * rg_0_simplex) + (t[..., None] * rg_1_simplex) # (B,L,K)
                rg_t_simplex = torch.where(batch['generate_mask'][...,None],rg_t_simplex,rg_1_simplex)
                rg_t_prob = F.softmax(rg_t_simplex,dim=-1) # (B,L,K)
                rg_t = sample_from(rg_t_prob) # (B,L)
                rg_t = torch.where(batch['generate_mask'],rg_t,rg_gen)
            else:
        ...
        # denoise
        pred_rotmats_1, pred_trans_1, pred_rg_prob, pred_bb_angle, pred_sc_angle  = self.ga_encoder.forward(
            t=t, rotmats_t=rotmats_t, trans_t = trans_t_c, 
            sc_angles_gen_t=sc_angles_gen_t, sc_angles_context=sc_angles_context, bb_angles_t=bb_angles_t,
            rg_gen_t=rg_t, rg_context=rg_context,
            node_embed=node_embed, edge_embed=edge_embed,
            generate_mask=gen_mask, res_mask=res_mask, ar_time=ar_time)
        
        pred_rg_1 = sample_from(F.softmax(pred_rg_prob,dim=-1))
        pred_rg_1 = torch.where(batch['generate_mask'],pred_rg_1,torch.clamp(rg_gen,0,19))
        ...
        # seqs vf loss
        seqs_loss = F.cross_entropy(pred_rg_prob.view(-1,pred_rg_prob.shape[-1]),
                                    rg_gen.view(-1), reduction='none'
                                    ).view(pred_rg_prob.shape[:-1]) # (N,L), not softmax
        seqs_loss = torch.sum(seqs_loss * gen_mask, dim=-1) / (torch.sum(gen_mask,dim=-1) + 1e-8)
        seqs_loss = torch.mean(seqs_loss)
        
        angle_mask_loss = batch['generate_mask']
        bb_angle_loss = self.calc_angle_loss(bb_angles_t, bb_angles, pred_bb_angle, norm_scale, angle_mask_loss)
        sc_angle_loss = self.calc_angle_loss(sc_angles_gen_t, sc_angles_gen, pred_sc_angle, norm_scale, angle_mask_loss)
        
        angle_loss = bb_angle_loss + sc_angle_loss
    ...
    def sample(self, batch, num_steps = 100, sample_bb=True, sample_ang=True, sample_seq=True):
        ...
        DEVICE = batch['aa'].device
        angle_mask_loss = torsions_mask.to(DEVICE)
        ...
        # angles_1 = torch.where(batch['generate_mask'][...,None],angles_1,torus.tor_random_uniform(angles_1.shape, device=DEVICE, dtype=angles_1.dtype))
        ...
        if sample_bb:
            ...
            trans_0 = torch.randn((num_batch,num_res,3), device=DEVICE) # scale with sigma?
        ...
        if sample_ang:
            # angle noise
            angles_0 = torus.tor_random_uniform(angles_1.shape, device=DEVICE, dtype=angles_1.dtype) # (B,L,5)
            angles_0 = torch.where(batch['generate_mask'][...,None],angles_0,angles_1)
        else:
            angles_0 = angles_1.detach().clone()
        if sample_seq:
            seqs_0_simplex = k * torch.randn((num_batch,num_res,K), device=DEVICE)
        ...
        # ts = torch.linspace(1.e-2, 0.9, num_steps)
        # ts = torch.linspace(0.1, 0.9, num_steps)
        # ts = [0] + ts.tolist()
        ...
        for t_2 in ts[1:]:
            ...
            clean_traj.append({'rotmats':pred_rotmats_1.cpu(),'trans':pred_trans_1_c.cpu(),
        ...
        pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob = self.ga_encoder(
            t, rotmats_t_1, trans_t_1_c, angles_t_1, seqs_t_1, node_embed, edge_embed, batch['generate_mask'].long(), batch['res_mask'].long())
        pred_rotmats_1 = torch.where(batch['generate_mask'][...,None,None],pred_rotmats_1,rotmats_1)
        # move center
        # pred_trans_1_c,center = self.zero_center_part(pred_trans_1,gen_mask,res_mask)
        pred_trans_1_c = torch.where(batch['generate_mask'][...,None],pred_trans_1,trans_1_c) # move receptor also
        ...
        if not sample_seq:
            ...
            pred_seqs_1_simplex = seqs_1_simplex.detach().clone()
        clean_traj.append({'rotmats':pred_rotmats_1.cpu(),'trans':pred_trans_1_c.cpu(),
                           'angles':pred_angles_1.cpu(),'seqs':pred_seqs_1.cpu(),'seqs_simplex':pred_seqs_1_simplex.cpu(),
                                'rotmats_1':rotmats_1.cpu(),'trans_1':trans_1_c.cpu(),'angles_1':angles_1.cpu(),'seqs_1':seqs_1.cpu()})
        return clean_traj
