...
import probayes.modules.protein.constants as constants
...
for i in range(21):
    torsions_mask[i] = torch.tensor([True] + constants.chi_angles_mask[i]).float() # check this

if __name__ =='__main__':
    test_pdb_path = '/data/wuhl/bfn4pep/probayes_data/pepbdb_probayes_processed/pdbs/1a1n_C/receptor.pdb'
    ...
    from openfold.data.data_transforms import get_chi_angles as openfold_get_chi_angles
    from remote.RDE_PPI.rde.utils.protein.parsers import parse_pdb as rde_parse_pdb
    from probayes.data.all_atom import compute_allatom
    from probayes.data import utils as du
    from openfold.np import residue_constants
    from openfold.np.residue_constants import MAP_HHBLITS_AATYPE_TO_OUR_AATYPE
    
    aatype_convention_map = torch.tensor(MAP_HHBLITS_AATYPE_TO_OUR_AATYPE)
    GROUP_IDX = torch.tensor(residue_constants.restype_atom14_to_rigid_group)
    # import AA
    from probayes.modules.protein.constants import AA
    torch.set_printoptions(sci_mode=False)  # 禁用科学计数法
    test_pep = parse_pdb(test_pdb_path)[0]
    rde_pep = rde_parse_pdb(test_pdb_path,model_id=0)[0]
    
    center = torch.sum(rde_pep['pos_heavyatom'] [rde_pep['mask_heavyatom'][:, BBHeavyAtom.CA], BBHeavyAtom.CA], dim=0)\
                / (torch.sum(rde_pep['mask_heavyatom'][:, BBHeavyAtom.CA]) + 1e-8)

    
    probayes_angles, torsion_mask = get_torsion_angle(test_pep['pos_heavyatom'],test_pep['aa'])
    # angles = torch.cat([rde_pep['omega'].unsqueeze(-1),
    #                     rde_pep['phi'].unsqueeze(-1), 
    #                     rde_pep['psi'].unsqueeze(-1),
    #                     rde_pep['chi']],dim=-1)
    angles = torch.cat([rde_pep['psi'].unsqueeze(-1),
                        rde_pep['chi']],dim=-1)
    rde_angles = angles % (2*math.pi)
    print(rde_angles.allclose(probayes_angles,atol=1e-3))

    rotmats_1 =  construct_3d_basis(rde_pep['pos_heavyatom'][ :, BBHeavyAtom.CA],
                                    rde_pep['pos_heavyatom'][ :, BBHeavyAtom.C],
                                    rde_pep['pos_heavyatom'][ :, BBHeavyAtom.N] )
    trans_1 = rde_pep['pos_heavyatom'][ :, BBHeavyAtom.CA]

    atom14_pos = compute_allatom(bb_rigids=du.create_rigid(rots=rotmats_1,trans=trans_1),
                                 angles=angles,aatype=rde_pep['aa'])
    # atom14_pos2 = compute_allatom(bb_rigids=du.create_rigid(rots=rotmats_1,trans=trans_1),
    #                              angles=angles2,aatype=rde_pep['aa'])
    gt_atom14_pos = rde_pep['pos_heavyatom'][:,:14,:]
    atom14_mask = rde_pep['mask_heavyatom'][:,:14].gt(0)
    cons_pos = atom14_pos[atom14_mask]
    gt_pos = gt_atom14_pos[atom14_mask]
    
    print((atom14_pos-gt_atom14_pos).abs().mean())
    
    # print(probayes_chi_angles)
    # print(rde_chi_angles)
    # print(probayes_chi_angles.allclose(rde_chi_angles%(2*math.pi), atol=1e-3))
    # print((probayes_chi_angles-rde_chi_angles).abs().max())
    # openfold_protein = from_pdb_string(open(test_pdb_path).read())
    # protein_features = make_protein_features(openfold_protein, description='test')
    # print(protein_features['all_atom_positions'].shape)
    # protein_features_torsion = atom37_to_torsion_angles_nocurry(protein_features)
    # print(protein_features_torsion.keys())

    # openfold_protein = openfold_get_chi_angles(protein_features_torsion)
    # print(openfold_protein['chi_angles_sin_cos'])