...
def get_side_chain_metrics(ref_angles,pred_angles,gen_mask,tor_mask):
    torsion_mask = tor_mask.to(ref_angles.device).unsqueeze(0).repeat(pred_angles.shape[0],1,1)
    generate_mask = gen_mask.unsqueeze(0).unsqueeze(-1).to(ref_angles.device)
    torsion_mask = torsion_mask * generate_mask
    freqs = torch.arange(-10,11).to(ref_angles.device).unsqueeze(0).unsqueeze(0)*np.pi*2
    tar_angle = ref_angles.unsqueeze(-1) + freqs
    chi_abs = (pred_angles.unsqueeze(-1) - tar_angle).abs().min(dim=-1).values
    # chi_abs = torch.min(chi_abs, 2*torch.pi - chi_abs)
    chi_abs = chi_abs*torsion_mask.float()*180/torch.pi

    psi_maes = chi_abs[torsion_mask[:,:,0]][:,0].mean()
    chi1_maes = chi_abs[torsion_mask[:,:,1]][:,1].mean()
    chi2_maes = chi_abs[torsion_mask[:,:,2]][:,2].mean()
    chi3_maes = chi_abs[torsion_mask[:,:,3]][:,3].mean()
    chi4_maes = chi_abs[torsion_mask[:,:,4]][:,4].mean()
    # TODO: refine this
    corr = (((chi_abs<=20).float()*torsion_mask.float()).sum() / (torsion_mask.sum()+1e-10)).cpu()
    return {
        'mae_psi':psi_maes,
        'mae_chi1':chi1_maes,
        'mae_chi2':chi2_maes,
...
# chi_abs = torch.abs(chain1['torsion']-chain2['torsion'])
# chi_abs = torch.min(chi_abs, 2*torch.pi - chi_abs)
# chi_abs = chi_abs*torsion_mask.float()*180/torch.pi

# for i in range(len(chi_abs)):
#     chi_mse = chi_abs.sum(dim=0) / (torsion_mask.sum(dim=0)+1e-10)
#     mses.append(chi_mse)
#     # sc_acc
#     sc_acc = ((chi_abs<=20).float()*torsion_mask.float()).sum() / (torsion_mask.sum()+1e-10)

from typing import Optional
from functools import partial
import math
from probayes.utils.so3_helpers import pairwise_geodesic_distance
import ot as pot
...
def get_so3_metrics(ref_rotmats, gen_rotmats,gen_mask):
    mask = gen_mask.gt(0)
    ref = ref_rotmats[mask, :]
    gen = gen_rotmats[mask, :]
    wdist = so3_wasserstein(ref,gen, power=1)
    return {
        'so3_wasserstein': wdist
    }