...
class DiscreteAccuracySchedule:
    def __init__(self, n_steps, beta1, n_dim, K, device='cuda:0'):
    ...
    def discrete_entropy(self, theta):
        return Categorical(probs=theta).entropy()
    ...
    def linear_entropy(self, step):
        ...
        theta0 = torch.tensor([1/self.K]*self.K,device=self.device)
        entropy0 = self.discrete_entropy(theta0)
        
        theta1 = torch.zeros_like(theta0)
        theta1[0] = 1
        entropy1 = self.discrete_entropy(theta1)
    ...
    def alpha_equation(self, prior_alphas, prior_beta, tar_beta, n_samples):
        def func(alpha):
            # prior_mean = torch.tensor(uniform_direction.rvs(dim=3,size=(n_samples)),device=self.device)
    ...
    def find_linear(self, n_samples=500000):
        ...
        for i in tqdm(range(1,self.n_steps)):
            ...
            root_alpha = root_scalar(self.alpha_equation(sender_alpha, prior_beta, target_beta, n_samples=n_samples),
...
if __name__ == '__main__':
    ...
    n_samples = 50000
    DEVICE = 'cuda:0'
    fname = f'./cache_files/discrete_schedules/discrete_linear_entropy_alphas_s{n_steps}_beta{int(beta1)}_dim{n_dim}.pt'
    find_linear = True

    acc_schedule = DiscreteAccuracySchedule(n_steps=n_steps,beta1=beta1,n_dim=n_dim,)
