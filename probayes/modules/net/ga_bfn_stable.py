...
class GAEncoder_BFN(nn.Module):
    def __init__(self, ipa_conf, bfn_conf):
        ...
        self.res_feat_mixer = nn.Sequential(
            nn.Linear(3 * self._ipa_conf.c_s + self.angles_embedder.get_out_dim(in_dim=5) + n_acc_dim, self._ipa_conf.c_s), # 5 is the dimension of log_acc_t
            nn.ReLU(),
            nn.Linear(self._ipa_conf.c_s, self._ipa_conf.c_s),
        )
    ...
    def forward(self, t, rotmats_t, trans_t, angles_t, seqs_t, node_embed, edge_embed, generate_mask, res_mask, log_acc_t):
        ...
        # pred_angles1 = self.angle_net(node_embed) + angles_t # residual connection
