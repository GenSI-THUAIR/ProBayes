...
def compute_allatom(bb_rigids, angles, aatype):
    '''
    By Hanlin
    Compute all atom positions from backbone frames and torsional angles.
    Args:
        bb_rigids: Backbone rigid groups. (n_batch, n_res)
        angles: Torsion angles. (n_batch, n_res, 5)
        aatype: Residue types. (n_batch, n_res)
    Returns:
        atom 14 positions.
    '''
    aatype_convention_map = torch.tensor(MAP_HHBLITS_AATYPE_TO_OUR_AATYPE).to(aatype.device)
    aatype_openfold = aatype_convention_map[aatype]

    # for phi, omega, fill zero which does not impact the result
    all_angles = torch.cat([torch.zeros_like(angles[..., :1]),
                              torch.zeros_like(angles[..., :1]), angles], dim=-1) # (n_batch, n_res, 7)
    # all_angles = angles
    # all_angles = torch.cat([0.5*torch.ones_like(angles[..., :1]), # these two value do not influence the results
    #                           0.3*torch.ones_like(angles[..., :1]), angles], dim=-1) # (n_batch, n_res, 7)
    torsion_angles = torch.stack([all_angles.sin(), all_angles.cos()], dim=-1) # (n_batch, n_res, 7, 2)
    all_frames = torsion_angles_to_frames(
        bb_rigids,
        torsion_angles,
        aatype_openfold,
    )
    atom14_pos = frames_to_atom14_pos(all_frames, aatype_openfold)
    return atom14_pos
