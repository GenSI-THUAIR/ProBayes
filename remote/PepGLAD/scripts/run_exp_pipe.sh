...
if [ "$TRAIN_AE_FLAG" = "1" ]; then
    EXPNAME=${NAME}_train_autoencoder
    echo "EXPNAME: $EXPNAME"
    bash scripts/train.sh $AECONFIG $EXPNAME --trainer.config.save_dir=$AE_SAVE_DIR 
fi
# exit 1
########## train ldm ##########
...
if [ "$TRAIN_LDM_FLAG" = "1" ]; then
   EXPNAME=${NAME}_train_ldm
#    echo "bash scripts/train.sh $LDMCONFIG $EXPNAME --trainer.config.save_dir=$LDM_SAVE_DIR --model.autoencoder_ckpt=$AE_CKPT"
   bash scripts/train.sh $LDMCONFIG $EXPNAME --trainer.config.save_dir=$LDM_SAVE_DIR --model.autoencoder_ckpt=$AE_CKPT 
fi

########## get latent distance ##########
LDM_CKPT=`cat ${LDM_SAVE_DIR}/version_9/checkpoint/topk_map.txt | head -n 1 | awk -F " " '{print $2}'`
...
if [ "$EVAL_FLAG" = "1" ]; then
    ...
    python cal_metrics.py --results ${LDM_SAVE_DIR}/version_9/results/results.jsonl >> $OUTLOG
fi
