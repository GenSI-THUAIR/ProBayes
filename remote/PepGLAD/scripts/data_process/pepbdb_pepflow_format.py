...
from remote.PepGLAD.data.mmap_dataset import create_mmap
from remote.PepGLAD.data.format import VOCAB
...
from remote.PepGLAD.data.converter.blocks_interface import blocks_interface, blocks_cb_interface
from remote.PepGLAD.utils.logger import print_log
from tqdm import tqdm

# python -m scripts.data_process.pepbdb_probayes_format --index ./datasets/pepbdb/peptidelist.txt --out_dir /data/wuhl/bfn4pep/probayes_data/pepbdb_probayes_processed

def parse():
    parser = argparse.ArgumentParser(description='Process PepBDB dataset')
    parser.add_argument('--index', type=str, required=True, help='Index file of the dataset')
...
def process_iterator(items, pdb_dir, pdb_out_dir, pocket_th):
    ...
    for index, (pdb_id, _)  in enumerate(tqdm(items.items(),desc='Processing PDBs')):
        ...
        pdb_id_path = os.path.join(pdb_out_dir, pdb_id)
        # create pdb_id directory
        ...
        try:
            ...
            if os.path.exists(pdb_id_path):
                for file in os.listdir(pdb_id_path):
                    os.remove(os.path.join(pdb_id_path, file))
                os.rmdir(pdb_id_path)
            print(e)
            continue
