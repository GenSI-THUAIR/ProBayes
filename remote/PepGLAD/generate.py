...
def save_data(
        _id, n,
        x_pkl_file, S,
        ref_pdb, rec_chain, lig_chain, 
        ref_save_dir, cand_save_dir,
        ref_lig_save_path, cand_lig_save_path,
    ...
    X = pkl.load(open(x_pkl_file, 'rb'))
    pmetric = 0
    os.remove(x_pkl_file)
    if seq_only:
        X = None
    elif struct_only:
        S = None
    ...
    return {
            ...
            'ref_lig_path': ref_lig_save_path,
            'cand_lig_path': cand_lig_save_path
    }
...
def save_data_prev(
    ...
    return {
            'id': _id,
            'number': n,
            'gen_pdb': os.path.abspath(gen_pdb),
