...
import sys
# sys.path.append('/AIRvePFS/ai4science/users/wuhl/bfn4pep/remote/PepGLAD')

import remote.PepGLAD.models as models
from remote.PepGLAD.utils.config_utils import overwrite_values
...
from remote.PepGLAD.data.format import VOCAB, Atom
from remote.PepGLAD.data import create_dataloader, create_dataset
from remote.PepGLAD.utils.logger import print_log
from remote.PepGLAD.utils.random_seed import setup_seed
from remote.PepGLAD.utils.const import sidechain_atoms
...
def generate_wrapper(model, sample_opt={}, use_dataset=True):
    def wrapper(batch):
        return batch['X'], batch['S']
...
def save_data_prev(
    ...
    return {
            'id': _id,
            'number': n,
            'gen_pdb': os.path.abspath(ref_pdb),
            'ref_pdb': os.path.abspath(ref_pdb),
            'pmetric': pmetric,
            'rec_chain': rec_chain,
            'lig_chain': lig_chain,
            'ref_seq': ref_seq,
...
def main(args, opt_args):
    ...
    fout = open(os.path.join(save_dir, 'results_dataset.jsonl'), 'w')
    item_idx = 0
    inputs = []
    ...
    for i in trange(len(test_set)):
        for n in range(n_samples):
            _id, ref_pdb, rec_chain, lig_chain = test_set.get_summary(i)
            X = test_set[i]['X']
            rec_blocks, lig_blocks = pdb_to_list_blocks(ref_pdb, selected_chains=[rec_chain, lig_chain])
            S = ''.join([VOCAB.abrv_to_symbol(block.abrv) for block in lig_blocks])
            pmetric = 0
    ...
    results = pool.starmap(save_data_prev, inputs)
