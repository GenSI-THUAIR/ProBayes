...
from remote.PepGLAD.evaluation import diversity
from remote.PepGLAD.evaluation.dockq import dockq
# from remote.PepGLAD.evaluation.rmsd import compute_msd
from remote.dyMEAN.evaluation.rmsd import compute_rmsd
from remote.PepGLAD.utils.random_seed import setup_seed
from remote.PepGLAD.evaluation.seq_metric import aar, slide_aar
...
def _get_ref_ligand_pdb(_id, dataset_pdb_dir, rec_chain, lig_chain):
    # search directory dataset_pdb_dir with name including _id
    return os.path.join(dataset_pdb_dir, f'{rec_chain}_{lig_chain}_pdb{_id}', f'peptide.pdb')

def _get_gen_ligand_pdb(_id, dataset_pdb_dir, rec_chain, lig_chain, n):
    return os.path.join(dataset_pdb_dir, f'{rec_chain}_{lig_chain}_pdb{_id}', f'{_id}_pep_packed_{n}.pdb')
...
def cal_metrics(items):
    ...
    for item in items:
        ...
        rmsd = compute_rmsd(ref_ca_x[ca_mask], gen_ca_x[ca_mask], aligned=False)
        ...
        if struct_only:
            ...
            results['RMSD(full-atom)'].append(compute_rmsd(
                np.array(gen_all_x), np.array(ref_all_x), aligned=False
            ))
    ...
    for name in metrics[0]:
        ...
        if isinstance(vals[0], dict):
            ...
            if '>=' in name or '<=' in name:  # percentage
                ...
                log_metrics[f'{name}(lowest)'] = aggr_vals[lowest_i]
                log_metrics[f'{name}(highest)'] = aggr_vals[highest_i]
        else:
            print(f'{name} (mean): {sum(vals) / len(vals)}')
            log_metrics[f'{name}(mean)'] = sum(vals) / len(vals)
            ...
            log_metrics[f'{name}(lowest)'] = vals[lowest_i]
            log_metrics[f'{name}(highest)'] = vals[highest_i]
...
def run_cal_metrics(results, num_workers=8, rosetta=False, filter_dG=None):
    '''
    results: str, path to the test set
    num_workers: int, number of workers to use
    rosetta: bool, use the rosetta-refined structure
    filter_dG: str, path to the dG filter
    '''
    args = argparse.Namespace(results=results, num_workers=num_workers, rosetta=rosetta, filter_dG=filter_dG)
    metrics = main(args)
    return metrics
