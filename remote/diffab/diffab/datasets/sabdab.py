...
def _label_heavy_chain_cdr(data, seq_map, max_cdr3_length=100, numbering_system='imgt'):
    ...
    for position, idx in seq_map.items():
        resseq = position[1]
        if numbering_system == 'imgt':
            cdr_type = constants.IMGTCDRRange.to_cdr('H', resseq)
        else:
...
def _label_light_chain_cdr(data, seq_map, max_cdr3_length=30, numbering_system='imgt'):
    if data is None or seq_map is None:
        return data, seq_map
    cdr_flag = torch.zeros_like(data['aa'])
    for position, idx in seq_map.items():
        resseq = position[1]
        if numbering_system == 'imgt':
            cdr_type = constants.IMGTCDRRange.to_cdr('L', resseq)
        else:
...
def preprocess_sabdab_structure(task, numbering_system='clothia'):
    entry = task['entry']
        ...
        if entry['H_chain'] is not None:
            if numbering_system == 'imgt':
                (
                    ...
                    max_resseq = 150    # Chothia, end of Heavy chain Fv
        ...
        if entry['L_chain'] is not None:
            if numbering_system == 'imgt':
                (
                    ...
                    max_resseq = 150    # Chothia, end of Light chain Fv,
                ), 
                numbering_system=numbering_system)
            else:
                (
