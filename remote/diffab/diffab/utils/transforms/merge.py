...
class MergeChains(object):
    ...
    def __call__(self, structure):
        ...
        data_out['torsion_angle'] = torch.cat([data_out['psi'].unsqueeze(-1),data_out['chi']],dim=-1)
        data_out['torsion_angle_mask'] = torch.cat([data_out['psi_mask'].unsqueeze(-1),data_out['chi_mask']],dim=-1)
        data_out['generate_mask'] = data_out['generate_flag'].detach().clone()
