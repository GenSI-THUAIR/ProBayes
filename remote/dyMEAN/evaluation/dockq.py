...
from remote.dyMEAN.data.pdb_utils import AgAbComplex, Protein
from remote.dyMEAN.utils.time_sign import get_time_sign
# from configs import DOCKQ_DIR, CACHE_DIR

DOCKQ_DIR = '/data/wuhl/bfn4pep/remote/PepGLAD/evaluation/DockQ'
CACHE_DIR = '/data/wuhl/bfn4pep/remote/dyMEAN/cache'
...
def dockq(mod_cplx: AgAbComplex, ref_cplx: AgAbComplex, cdrh3_only=False):
    ...
    if cdrh3_only:
        ...
        p = os.popen(f'/data/wuhl/anaconda3/envs/flow/bin/python {os.path.join(DOCKQ_DIR, "DockQ.py")} {mod_pdb} {ref_pdb} -native_chain1 {H} -no_needle')
    else:
        mod_cplx.to_pdb(mod_pdb)
        ref_cplx.to_pdb(ref_pdb)
        p = os.popen(f'/data/wuhl/anaconda3/envs/flow/bin/python {os.path.join(DOCKQ_DIR, "DockQ.py")} {mod_pdb} {ref_pdb} -native_chain1 {H} {L} -no_needle')
    text = p.read()
    p.close()
