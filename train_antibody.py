...
from probayes.eval.antibody_eval_single import get_metrics_antibody
...
from remote.diffab.diffab.datasets import get_dataset
...
if __name__ == '__main__':
    ...
    transforms = get_transform(config.dataset.train.transform)

    train_dataset = AntibodyDataset(structure_dir = config.dataset.train.structure_dir, 
    ...
    val_dataset = AntibodyDataset(structure_dir = config.dataset.val.structure_dir, dataset_dir = config.dataset.val.dataset_dir,
                                            split_json_path= config.dataset.val.split_json_path,
                                            name = config.dataset.val.name, transform=get_transform(config.dataset.val.transform), reset=config.dataset.val.reset)
    ...
    # TODO: to be deleted
    test_dataset = AntibodyDataset(structure_dir = config.dataset.test.structure_dir, 
    ...
    test_loader = DataLoader(test_dataset, batch_size=min(config.train.batch_size,len(test_dataset)), shuffle=False, collate_fn=PaddingCollate(), num_workers=args.num_workers)
    
    train_batch_size = config.train.batch_size if local_rank!=0 else int(config.train.batch_size*0.7)
    
    # train_sampler = DistributedSampler(train_dataset, shuffle=True)
    train_loader = DataLoader(train_dataset, batch_size=train_batch_size, collate_fn=PaddingCollate(), num_workers=args.num_workers, pin_memory=True)
    ...
    def validate(it):
        ...
        with torch.no_grad():
            ...
            # TODO: to be modifed
            metrics = get_metrics_antibody(ckpt_dir=ckpt_dir, model=val_model, dataset=test_dataset,
    ...
    def average_metric(metrics):
        ...
        return metrics['AAR'] + metrics['DockQ'] + metrics['LDDT'] + metrics['TMscore']  - metrics['RMSD(CA) CDRH3']
                ...
                if config.train.run_gen:
                    ...
                    if val_metrics is not None and average_metric(val_metrics) > best_score:
                        ...
                        score_str = '-'.join([key+'-'+f'{val:.3f}' 
                                              ...
                                              if key in ['AAR','DockQ','LDDT','RMSD(CA) CDRH3']])
